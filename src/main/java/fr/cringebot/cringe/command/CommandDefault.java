...
public class CommandDefault {
	...
	private void top(Message msg){
		...
		sb.append(squads.get(0).getName()).append(" ").append(squads.get(0).getTotal()).append(" ").append(msg.getGuild().getMemberById(squads.get(0).getBestid()).getAsMention());
		sb.append(squads.get(1).getName()).append(" ").append(squads.get(1).getTotal()).append(" ").append(msg.getGuild().getMemberById(squads.get(1).getBestid()).getAsMention());
		sb.append(squads.get(2).getName()).append(" ").append(squads.get(2).getTotal()).append(" ").append(msg.getGuild().getMemberById(squads.get(2).getBestid()).getAsMention());
	...
	private void waifu(Message msg) throws ExecutionException, InterruptedException {
		...
		if (msg.getMember().getId().equals("420655502276558848"))
			WaifuCommand.CommandMain(msg);
		else
			msg.getChannel().sendMessage("aparemment j'ai un probleme, je suis actuellement en train d'etre examiné").queue();
	}
