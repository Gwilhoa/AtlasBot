...
public class ProfilCommand {
    public static EmbedBuilder CommandProfil(Member member){
        ...
        EmbedBuilder eb = new EmbedBuilder().setColor(mem.getSquad().getColor())
                ...
                .addField("> Succès :", ach.size() +" / " + achs.size(), true)
                .addField("> Memes approuvés :", mem.getBestmeme().toString(), true)
        ...
        if (mem.getTitle() != null) {
            eb.setTitle("Profil de __" + mem.getTitle() + "__ " + mem.getName());
        } else {
            eb.setTitle("Profil de " + mem.getName());
        }
        return eb;
    }
